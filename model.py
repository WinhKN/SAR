...
class Trainer:
    ...
    def cluster_loss(self, cur_left, cur_right, timestamp_list, label_num, middle_pred):
        ...
        for vidx in range(len(timestamp_list)):
            num_timestamps = len(timestamp_list[vidx])
            num_left = len(cur_left[vidx])
            limit = min(num_timestamps, num_left)  # tránh vượt giới hạn

            for i in range(limit):
                t = timestamp_list[vidx][i]
                if t >= middle_pred.size(-1):
                    continue
                center_vec = middle_pred[vidx, :, t]
